...
static ChessResult setPlayerResults(Map Players, Tournament tournament, int id, int time, int id_winner)
{
	...
	{
		playerSetNumLoses(player_result, 1 + getNumberLoses(player_result));
	}
...
ChessSystem chessCreate()
{
	...
	chess->player_results = mapCreate(playerCopy, (copyMapKeyElements)copyKey,
									  playerDestroy, (freeMapKeyElements)freeKey, (compareMapKeyElements)compareKeys);
	return chess;
}
...
ChessResult chessAddTournament(ChessSystem chess, int tournament_id, int max_games_per_player, const char* tournament_location)
{
	...
	Map PlayerStatuses = mapCreate(playerCopy, (copyMapKeyElements)copyKey,
								   playerDestroy, (freeMapKeyElements)freeKey,
